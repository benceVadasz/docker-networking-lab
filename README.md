...
Docker is used to run a **public/private network topology**: a reverse proxy (NGINX) as the single entry point on the public side, and a frontend (Vite), backend (Node), and database (Postgres) on the private side. All user traffic goes through the proxy; the backend and database are not directly exposed to the host.
